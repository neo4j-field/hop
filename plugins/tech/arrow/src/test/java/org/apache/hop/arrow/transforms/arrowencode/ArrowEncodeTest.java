...
public class ArrowEncodeTest {
  ...
  public void testEncodingSingleBatch() throws HopException {
    ...
    meta.setSourceFields(
        List.of(new SourceField("age", "age_out"), new SourceField("name", "name_out")));
    ...
    Assert.assertEquals("First vector should be renamed age_out", vectors[0].getName(), "age_out");
    Assert.assertEquals(
        "First vector should be renamed name_out", vectors[1].getName(), "name_out");
